...
fn main() {
    ...
    let number_one: Box<i32> = Box::new(491);
    let number_two: Box<i32> = Box::new(59);
    let number_three:Box<i32> = Box::new(*number_one + *number_two);

    println!("value of number_three is: {}", *number_three);
